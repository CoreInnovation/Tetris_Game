...
(function (Arcade) {
  ...
  class Pong {
    ...
    update(dt, now) {
      ...
      // ---- serve countdown ----
      if (this.serveT > 0) { this.serveT -= dt; if (this.serveT <= 0) this._launch(this._serveDir); else { /* CPU idles toward center */ } }

      // ---- CPU paddle: tracks the ball with capped speed + error (beatable) ----
      const cpuSpeed = this.pSpeed * (0.62 + Math.min(0.5, this.points * 0.035));   // ramps with your score, always < player
      const err = Math.max(6, (this.pw * 0.42) - this.points * 2);                  // big sloppy zone early, tightens later
      const aimX = (this.ball.vy < 0 ? this.ball.x : c.x + c.w / 2) + (this._cpuErr || 0);
      if (Math.random() < dt / 600) this._cpuErr = rand(-err, err);                 // re-roll aim wobble occasionally
      const cd = clamp(aimX - this.cpuP.x, -cpuSpeed * dt, cpuSpeed * dt);
      this.cpuP.x = clamp(this.cpuP.x + cd, minX, maxX);

      if (this.serveT > 0) return;   // ball not in play yet
      ...
      // player paddle (bottom) — ball heading down
      if (b.vy > 0 && b.y + b.r >= this.playerY && b.y + b.r <= this.playerY + this.ph + Math.abs(b.vy * dt) && Math.abs(b.x - this.player.x) <= this.pw / 2 + b.r) {
        ...
        this.score += 1; this.rally++;
      }
      // cpu paddle (top) — ball heading up
      ...
      // scoring
      if (b.y - b.r > c.y + c.h) { this._miss(); }              // passed player -> CPU scores
      else if (b.y + b.r < c.y) { this._scorePoint(); }         // passed CPU -> you score!
    }
    ...
    _drawHud(ctx, th, now) {
      ...
      ctx.textBaseline = "bottom"; ctx.fillText(String(this.cpu), c.x + c.w * 0.5, c.y + c.h / 2 - 8);
      ctx.textBaseline = "top"; ctx.fillText(String(this.points), c.x + c.w * 0.5, c.y + c.h / 2 + 8);
      ctx.globalAlpha = 1;
      ...
      // lives as dots near the player
      const dr = Math.max(4, this.br * 0.7);
      for (let i = 0; i < START_LIVES; i++) { ctx.fillStyle = i < this.lives ? pal.player : "rgba(255,255,255,0.18)"; ctx.beginPath(); ctx.arc(c.x + c.w - 14 - i * (dr * 2.6), 8 + dr, dr, 0, Math.PI * 2); ctx.fill(); }
      // serve "GET READY"
